...
public class AuctionChart extends JFrame
{
    ...
    public AuctionChart(final String title, final String xAxisLabel, final String yAxisLabel, List <ProfitItem> pi) {

        super(title);

        final XYDataset dataset = createDataset(pi, title);
        final JFreeChart chart = createChart(dataset, title, xAxisLabel, yAxisLabel);
        final ChartPanel chartPanel = new ChartPanel(chart);
        chartPanel.setPreferredSize(new java.awt.Dimension(800, 390));
        setContentPane(chartPanel);
    ...
    private XYDataset createDataset(List <ProfitItem> pi, String title) {
        final XYSeries series2 = new XYSeries(title);
        for (int i = 0; i<pi.size(); i++)
        {
            series2.add(pi.get(i).getPrice(), pi.get(i).getProfit());
        }
...
//        final StandardLegend legend = (StandardLegend) chart.getLegend();
  //      legend.setDisplaySeriesShapes(true);
    ...
    private JFreeChart createChart(final XYDataset dataset, final String title, final String xAxisLabel, final String yAxisLabel) {
